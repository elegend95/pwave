...
breve recap della tesi:

la mia tesi consiste nello studio di un metodo sperimentale semplice di rivelazione di quasiparticelle con statistica anyonica. Ci concentreremo sullo studio di superfluidi p-wave, la piattaforma di realizazione fisica è un gas di atomi freddi intrappolati.

Questi anyoni sono quasiparticelle che possono realizzarsi solamente in sistemi 2d gappati come eccitazioni collettive e variano la loro natura a secondo se lo stato in cui appaiono sia degenerato oppure o no. In 4D non ci sono per il teorema di spin e statistica.

Fino ad adesso lo studio di questi sistemi è avvenuto solamente in sistemi a stato solido tipo kitaev ma non c'è sicurezza sperimentale.

L'idea fondamentale testata inizialmente su stati FQHE è (fai i conti) che se ruoto rigidamente un anyone posso prendere tre contributi: dinamico, cambio sistema di riferimento, geometrico.

Il geometrico è aharonov bohm + termine topologico. Possiamo isolare il termine topologico sottraendo via il termine geometrico mettendo uno sopra all'altro i due anyoni.

Questo lavoro è stato fatto solamente per funzioni d'onda paradigmatche (moore read and laughlin) perchè il sistema non ha una hamiltoniana banale. Noi lo facciamo con hamiltoniane che conosciamo che ci permettono di inserire difetti come ci pare, boundaries come ci pare e magari pure accendere la temperatura. Inoltre a livello computazionale è molto più facile perchè gli autovalori scalano quadraticamente.
